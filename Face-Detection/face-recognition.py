...
ui, _ = loadUiType('Face_recognition.ui')
...
class MainApp(QMainWindow, ui):
    ...
    def show_selected_date_reports(self):
        ...
        con = sqlite3.connect("face-reco.db")
        cursor = con.execute(
            "SELECT * FROM attendance WHERE attendancedate = '" + str((self.dateEdit.date()).toPyDate()) + "'")
        result = cursor.fetchall()
        r = 0
        c = 0
        for row_number, row_data in enumerate(result):
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                if (prediction[1] < 800):
                    ...
                    self.currentprocess.setText("Dected face " + names[prediction[0]])
                    ...
                    try:
                        con = sqlite3.connect("face-reco.db")
                        cursor = con.execute("SELECT * FROM attendance WHERE name='" + str(
                            names[prediction[0]]) + "' and attendancedate = '" + str(date.today()) + "'")
                        result = cursor.fetchall()
                        if result:
                            available = True
                        if (available == False):
                            con.execute("INSERT INTO attendance VALUES(" + str(attendanceid) + ",'" + str(
                                names[prediction[0]]) + "','" + str(date.today()) + "')")
                            con.commit()
                    except:
                        print("Error in database insert")
                    print("Attendance Registered successfully")
                    self.currentprocess.setText("Attence entered for " + names[prediction[0]])
                    cnt = 0
